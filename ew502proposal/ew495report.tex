...
For my concept demonstrations, I will be modeling the rigid body dynamics of the quadrotor using parameters obtained from the Bitcraze website 
\textbf{Enter bitcraze citation here} %website: https://wiki.bitcraze.io/misc:investigations:thrust#thrust_measurement
...
%cite quadsim_68 or whatever it is
...
\begin{figure}
\begin{center}
\includegraphics[width=0.6\columnwidth]{\myroot/figures/2D_traj_10x.png}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.6\columnwidth]{\myroot/figures/2D_traj_20x.png}
\end{center}
...
Taking a look at the figures alone, it is easy to see that as the speed was reduced, the quadrotor was able to more closely follow the desired hummingbird trajectory. A look at the RMSE calculations in Table~\ref{table-RMSE}
%ref table 1 with RMSE values... add as a .png
...
%I suppose this counts as results??
Initial hardware testing for flying the Crazyflie and obtaining automatic 3D position tracking data via the OptiTrack was successful. The cfclient software was downloaded from the bitcraze website
%bitcraze reference here
 and a Logitec gamepad was used to send velocity and thrust commands to the Crazyflie through the Crazyradio to establish manual flight control. The OptiTrack system worked independently from this workstation, and recorded $x$, $y$, $z$ position data while logging the time $t$ that each data point was recorded at. The graphical results of one such trial are displayed in \fref{fig:manualFlight}. In general, the next step would be to upgrade from manual flight of the Crazyflie to autonomous flight.
...
\subsection{Budget}
\emph{\textbf{Please add the following disclaimer: ``*Disclaimer: Labor and overhead costs are estimated only for EW502 training purposes and do not actually reflect real costs that would be supported by project sponsors. Additionally, there are no new costs predicted for the spring semester. .''}}
